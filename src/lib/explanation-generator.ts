...
export class ExplanationGenerator {
  ...
  private createExplanationPrompt(keyword: string): string {
    ...
    return `中学受験の${subjectName}分野「${keyword}」について、中学受験生が確実に覚えられて得点につながる実用的な解説を作成してください。
...
【文字数】: 200-400文字（詰め込みすぎず、でも実用的に）

【例文構造】
${keyword}は[基本説明]。[背景・原因]のため重要です。
覚え方：[具体的な記憶テクニック]
試験では[出題パターン]として出題され、[注意点]に気をつけましょう。
関連：[関連知識]も合わせて覚えると効果的です。

【禁止事項】
- 「〜について学習しましょう」などの一般的な文言
- 表面的な説明のみ
- 暗記のコツがない解説
- 試験対策への言及がない解説
