...
console.log('🏛️ 歴史問題数をチェック中...');

const content = fs.readFileSync('./src/data/history.ts', 'utf-8');

// Extract questions with categories
const questionMatches = [...content.matchAll(/\{\s*id:\s*(\d+)[^}]*category:\s*['"]([^'"]*)['"]/gs)];

console.log(`📊 歴史総問題数: ${questionMatches.length}問`);
...
const expectedCategories = ['primitive', 'ancient', 'medieval', 'early-modern', 'modern', 'contemporary'];
let totalExpected = 0;
...
// Check if history.ts file exists
try {
  const stats = fs.statSync('./src/data/history.ts');
  console.log(`\n📁 history.ts ファイルサイズ: ${stats.size} bytes`);
} catch (error) {
  console.log('\n❌ history.ts ファイルが見つかりません');
}